...
def F(x, y):  # 香蕉函数
    """用于测试最优化算法的函数，具有一个全局最小值和多个局部最小值
       ...
       返回值: 函数的输出值，刻画当前(x, y)偏离最优解的程度。"""
    return (6.452*(x+0.125*y)*(cos(x)-cos(2*y))**2)/(0.8+(x-4.2)**2+2*(y-7)**2)+3.226*y
 
 
def getfitness(pop):  # 适应度函数
    """它根据个体的基因型（编码）计算出一个数值，表示该个体在解决问题上的表现。
       适应度函数的设计直接影响到遗传算法的搜索效率和最终结果。
       目的：评估个体在种群中的优劣程度。
       参数:
       pop: 一个二维数组，表示种群中的个体，每行代表一个个体的基因型（编码）。
       返回值: 一个一维数组，表示每个个体的适应度值，数值越大表示个体越适应环境。"""
    x, y = decodeDNA(pop)
    temp = F(x, y)
...
def decodeDNA(pop):  # 二进制转坐标，解码
    """它将个体的基因型（编码）转换为实际的解空间中的坐标值。
       目的：将个体的基因型（编码）转换为实际的解
       参数:
       pop: 一个二维数组，表示种群中的个体，每行代表一个个体的基因型（编码）。
...
def mutation(temp, MUTA_RATE):  # 变异
    """它对个体的基因型（编码）进行随机的修改，以引入新的基因变体，增加种群的多样性。
...
def crossmuta(pop, CROSS_RATE):  # 交叉
    """它将两个个体的基因型（编码）进行组合，产生新的个体，以模拟自然界中的遗传过程。
...
def print_info(pop):  # 输出最优解等
    """它根据当前种群中的个体的基因型（编码）计算出最优解的信息，并打印出来。
...
def plot_3d(ax):  # 建模
    """它在三维空间中绘制出函数F(x, y)的曲面图，以可视化函数的形状和最优解的位置。
...
if __name__=="__main__":  # 主函数
    ...
    # fig.add_axes(ax)
    #因为我的电脑上tcl库安装异常，无法显示窗口，所以我将运行结果直接输出成图片，不做成交互窗口。
    plot_3d(ax)
